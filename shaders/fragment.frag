...
void main()
{
    ...
    vec3 emissive_map = u_has_emissive ? pow(texture(u_emissive, v_uv).rgb, vec3(2.2)) * u_emissive_factor * u_emissive_strength : u_emissive_factor * u_emissive_strength;
    ...
    // get ao
    float ao_sample = u_has_ao ? texture(u_ao, v_uv).r : 1.0;
