...
{
    vec2 uv = v_uv;
    vec4 color = u_has_albedo ? texture(u_albedo, uv) * u_albedo_factor : u_albedo_factor;
    if(color.a < 0.1)
        discard;
    ...
    vec3 metal_rough = u_has_metallic_roughness ? texture(u_metallic_roughness, uv).rgb : vec3(0.0);
    ...
    vec3 normal = u_has_normal ? texture(u_normal, uv * u_normal_scale).rgb * 2.0 - 1.0 : vec3(0.0);
    normal = normalize(v_TBN * normal);

    // extract emissive maps and convert to srgb
    vec3 emissive_map = u_has_emissive ? pow(texture(u_emissive, uv).rgb, vec3(2.2)) * u_emissive_factor * u_emissive_strength : u_emissive_factor * u_emissive_strength;
    //emissive_map *= u_emissive_factor;

    // get ao maps
    //float ambient_occlusion = u_has_ao ? texture(u_ao, uv).r : 0.0;
