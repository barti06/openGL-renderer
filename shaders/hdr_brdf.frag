...
vec2 brdf_integrate(float NdotV, float roughness);

void main() 
{
    vec2 integratedBRDF = brdf_integrate(v_uv.x, v_uv.y);
...
vec2 brdf_integrate(float NdotV, float roughness)
{
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        vec3 H = ggx_imp_sample(Xi, N, roughness);
